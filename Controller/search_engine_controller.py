...
from Service.search_engine_service import process_search_engine
...
def validate_search_engine(params):
    try:
        logging.info("start of validate_search_engine function")
        optional_keys = {}
        missing = validate_request_body(params, optional_keys=optional_keys)
        if not missing:
            response = process_search_engine(params)
            return response
        else:
            query = """
        ...
        logging.info("customexception in validate_search_engine function")
        query = """
                INSERT INTO error_logs (error,file_name)
                VALUES (%s, %s);
            """
        ...
        logging.info("unknown exceptions in validate_search_engine function")
        query = """
                INSERT INTO error_logs (error,file_name)
                VALUES (%s, %s);
            """
