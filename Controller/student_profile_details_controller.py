...
from Service.student_profile_details_service import process_Student_profile_details
...
def validate_student_profile_details(params):
    try:
        logging.info("start of validate_student_profile_details function")
        optional_keys = {}
        missing = validate_request_body(params, optional_keys=optional_keys)
        if not missing:
            response = process_Student_profile_details(params)
            return response
        else:
            query = """
        ...
        logging.info("customexception in validate_student_profile_details function")
        query = """
                INSERT INTO error_logs (error,file_name)
                VALUES (%s, %s);
            """
        ...
        logging.info("unknown exceptions in validate_student_profile_details function")
        query = """
                INSERT INTO error_logs (error,file_name)
                VALUES (%s, %s);
            """
