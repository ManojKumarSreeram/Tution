...
from Service.update_student_details_service import process_student_updated_details
...
def validate_student_updated_details(params):
    try:
        logging.info("start of validate_student_updated_details function")
        optional_keys = {}
        missing = validate_request_body(params, optional_keys=optional_keys)
        if not missing:
            response = process_student_updated_details(params)
            return response
        else:
            query = """
        ...
        logging.info("customexception in validate_student_updated_details function")
        query = """
                INSERT INTO error_logs (error,file_name)
                VALUES (%s, %s);
            """
        ...
        logging.info("unknown exceptions in validate_student_updated_details function")
        query = """
                INSERT INTO error_logs (error,file_name)
                VALUES (%s, %s);
            """
