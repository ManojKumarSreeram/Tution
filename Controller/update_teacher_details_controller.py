...
from Service.update_teacher_details_service import process_teacheres_updated_details
...
def validate_teacheres_updated_details(params):
    try:
        logging.info("start of validate_teacheres_updated_details function")
        optional_keys = {}
        missing = validate_request_body(params, optional_keys=optional_keys)
        if not missing:
            response = process_teacheres_updated_details(params)
            return response
        else:
            query = """
        ...
        logging.info("customexception in validate_teacheres_updated_details function")
        query = """
                INSERT INTO error_logs (error,file_name)
                VALUES (%s, %s);
            """
        ...
        logging.info("unknown exceptions in validate_teacheres_updated_details function")
        query = """
                INSERT INTO error_logs (error,file_name)
                VALUES (%s, %s);
            """
