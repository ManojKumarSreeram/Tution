...
from Service.user_login_service import process_user_login_details
...
def validate_user_login_details(params):
    try:
        logging.info("start of validate_user_login_details function")
        optional_keys = {}
        missing = validate_request_body(params, optional_keys=optional_keys)
        if not missing:
            response = process_user_login_details(params)
            return response
        else:
            query = """
        ...
        logging.info("customexception in validate_user_login_details function")
        query = """
                INSERT INTO error_logs (error,file_name)
                VALUES (%s, %s);
            """
        ...
        logging.info("unknown exceptions in validate_user_login_details function")
        query = """
                INSERT INTO error_logs (error,file_name)
                VALUES (%s, %s);
            """
