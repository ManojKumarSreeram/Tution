...
def process_parent_profile_details(params):
    try:
        ...
        # student Details
        parent_details_query = "select parent_id,first_name, last_name, email,phone_number from parent_login where parent_id = %s and is_active= true;"
        values = (parent_id,)
        ...
        if parent_details_result:
            parent_formatted_data = {'id': parent_details_result[0], 'first_name': parent_details_result[1],'last_name': parent_details_result[2],'email': parent_details_result[3],'phone_number':parent_details_result[4]}
